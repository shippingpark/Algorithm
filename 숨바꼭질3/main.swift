...
func solution_숨바꼭질() {
  ...
  var dist = Array(repeating: Int.max, count: 100_001) // 목적지의 두 배까지 도달할 일은 없으므로
  dist[N] = 0
  ...
  while !pq.isEmpty {
    ...
    for i in [current, -1, 1] {
      ...
      guard 0..<100_001 ~= next else { continue }
      
      if i == current {
        if dist[next] > currentWeight {
          dist[next] = currentWeight
          pq.append((next, currentWeight))
        }
      } else {
        if dist[next] > currentWeight + 1 {
          dist[next] = currentWeight + 1
          pq.append((next, currentWeight + 1))
        }
      }
    }
  }
